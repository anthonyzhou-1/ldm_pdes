...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = zero_module(nn.Conv3d(out_channels, out_channels, kernel_size=(3, 3, 3), padding=(1, 1, 1)))
...
class Unet3D(nn.Module):
    ...
    def __init__(
        ...
        if use1x1:
            self.image_proj = nn.Conv3d(insize, n_channels, kernel_size=1)
        else:
            self.image_proj = nn.Conv3d(insize, n_channels, kernel_size=(3, 3, 3), padding=(1, 1, 1))
        ...
        out_channels = time_future * (self.n_output_scalar_components + self.n_output_vector_components * 3)
        if use1x1:
            self.final = zero_module(nn.Conv3d(in_channels, out_channels, kernel_size=1))
        else:
            self.final = zero_module(nn.Conv3d(in_channels, out_channels, kernel_size=(3, 3, 3), padding=(1, 1, 1)))

    def forward(self, x: torch.Tensor, emb=None):
        # x in shape b c d h w
        x = self.image_proj(x)
